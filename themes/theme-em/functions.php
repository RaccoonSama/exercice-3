<?php
/**
 * theme-em functions and definitions
 *
...
if ( ! function_exists( 'theme_em_setup' ) ) :
	/**
	 ...
	 */
	function theme_em_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on theme-em, use a find and replace
		 * to change 'theme-em' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'theme-em', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'theme-em' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'theme_em_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'theme_em_setup' );
 ...
 */
function theme_em_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'theme_em_content_width', 640 );
}
add_action( 'after_setup_theme', 'theme_em_content_width', 0 );
 ...
 */
function theme_em_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'theme-em' ),
			'id'            => 'sidebar-1',
...
function theme_em_scripts() {
	...
	wp_register_style( 'theme-em-style', get_stylesheet_uri(), array(), filemtime(get_template_directory() . "/style.css"), 'all' );
	wp_enqueue_style('theme-em-style');
	...
	wp_register_script( 'theme-em-carrousel', get_template_directory_uri() . '/js/carrousel.js', array(), filemtime(get_template_directory() . '/js/carrousel.js' ), true);
...
add_action( 'wp_enqueue_scripts', 'theme_em_scripts' );
